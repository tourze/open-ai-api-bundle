...
| ---- | ---- | -------- | ----------- |
| `thread_id` | string | Yes | The ID of the [thread](/docs/api-reference/threads) to which this run belongs. |
...
| Property | Type | Required | Default | Allowed Values | Description |
| -------- | ---- | -------- | ------- | -------------- | ----------- |
| `tool_call_id` | string | No |  |  | The ID of the tool call in the `required_action` object within the run object the output is being submitted for. |
| `output` | string | No |  |  | The output of the tool call to be submitted to continue the run. |
## Responses
